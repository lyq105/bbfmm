 #ifndef __Laplace_fem_3d__
 #define __Laplace_fem_3d__
...
// #include <mkl_lapacke.h>
 ...
 public:
	...
	double rhs_func(Point rpt){return 0;};
protected:
	int number_of_dof;
	Mesh mesh;
...
int Laplace_fem_3d::initial_fem_space(std::string filename)
{
	// load mesh;
	...
	{
		Laplace_fem_3d::Element_dof_map edof;
		Mesh::Volume_element& ve = mesh.find_volume_element(i);
		edof.number_of_e_dof = 4;
	...
	{
		Laplace_fem_3d::Surface_element_dof_map sedof;
		Mesh::Surface_element& se = mesh.find_surface_element(i);
		sedof.number_of_e_dof = 3;
...
int Laplace_fem_3d::assmble_matrix()
{
	...
	for ( int e_id = 0; e_id < number_of_dof; ++ e_id )
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			for (int j = 0; j < 4; ++j)
			{
				...
				{
					ke[i][j] += ( t3v.dshape_dx(i,0,t3v.gauss_int.p_list[g_id])*t3v.dshape_dx(j,0,t3v.gauss_int.p_list[g_id])
                      + t3v.dshape_dx(i,1,t3v.gauss_int.p_list[g_id])*t3v.dshape_dx(j,1,t3v.gauss_int.p_list[g_id]) 
						          + t3v.dshape_dx(i,2,t3v.gauss_int.p_list[g_id])*t3v.dshape_dx(j,2,t3v.gauss_int.p_list[g_id]) )
...
int Laplace_fem_3d::apply_bc()
{
	for (int b_id =0; b_id<bc.size();++b_id)
	{
		...
		{
			Mesh::Surface_element& se = mesh.find_surface_element(bc[b_id].index);
			Link2nodeSur l2s;
			l2s.initial(mesh,se);
			double fe[2] = {0};
			for (int i = 0; i < 4; ++i)
...
int Laplace_fem_3d::plot_result(std::string resfname)
{
